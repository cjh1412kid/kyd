...
Page({
  ...
  checkBtnClick: function (event) {
    ...
    let totalNum=current.data.totalNum;
  
    if (checkStatus == 1) {
      totalPrice += (clickData.totalSelectNum * clickData.salePrice);
      totalNum += clickData.totalSelectNum
    } else {
      totalPrice -= (clickData.totalSelectNum * clickData.salePrice);
      totalNum -= clickData.totalSelectNum;
    }
  ...
  gotoDetail:function(e){
    ...
    var shoesSeq = e.currentTarget.dataset.shoesseq;
    wx.navigateTo({
      url: '/pages/goods/detail/detail?seq='+shoesSeq,
    })
  },
...
function loadCartList(current) {
  app.gwRequest({ url: 'cart/cartList' }).then(res => {
    console.log(res)
    ...
    for (let index in res.result) {
      ...
      if (oneData.isChecked != 1) {
        ...
        totalPrice += (oneData.totalSelectNum * oneData.salePrice) ;
        totalNum += oneData.totalSelectNum 
      }
    }
