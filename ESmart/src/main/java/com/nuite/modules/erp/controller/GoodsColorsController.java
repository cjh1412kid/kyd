...
public class GoodsColorsController extends AbstractController {

    @RequestMapping("/v1.0")
    public Result getAllGoodsColors(HttpServletRequest request) {

        CommonErpService commonErpService = getErpService(request);

        List allGoodsColors = commonErpService.getAllGoodsColor();
        if (allGoodsColors == null || allGoodsColors.isEmpty()) {
            return Result.error(ResultStatus.DATA_NOT_EXIST.value(), ResultStatus.DATA_NOT_EXIST.getReasonPhrase());
        }
        return Result.ok(allGoodsColors);
