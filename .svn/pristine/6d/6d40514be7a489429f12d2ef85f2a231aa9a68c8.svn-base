...
public class MeetingGoodsServiceImpl extends ServiceImpl<MeetingGoodsDao, MeetingGoodsEntity> implements MeetingGoodsService {
    ...
    @Override
    public List<MeetingGoodsEntity> orderOfMeetingGoods(Integer meetingSeq,String year,Integer type,Integer status,BaseUserEntity userEntity,Page<MeetingGoodsEntity> page) throws Exception {
        Map<String,Object> map = new HashMap<>(10);
        map.put("meetingSeq",meetingSeq);
        map.put("year",year);
        map.put("type",type);
        map.put("status",status);
        map.put("userSeq",userEntity.getSeq());
        map.put("companySeq",userEntity.getCompanySeq());
        return meetingGoodsDao.orderOfMeetingGoods(map,page);
    }

    @Override
    public List<MeetingGoodsEntity> orderGoodsNum(Integer meetingSeq, String year, Integer type, Integer status, BaseUserEntity userEntity) throws Exception {
        Map<String,Object> map = new HashMap<>(10);
        map.put("meetingSeq",meetingSeq);
        map.put("year",year);
        map.put("type",type);
        ...
        map.put("userSeq",userEntity.getSeq());
        map.put("companySeq",userEntity.getCompanySeq());
	    return meetingGoodsDao.orderGoodsNum(map);
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public boolean decodeMeetingGoods(Integer meetingGoodsSeq, BaseUserEntity baseUserEntity) throws Exception {
	    //根据seq获取订货会货品
	    MeetingGoodsEntity meetingGoodsEntity = meetingGoodsDao.selectById(meetingGoodsSeq);
        meetingGoodsEntity.setIsDecode(1);
        return retBool(meetingGoodsDao.updateById(meetingGoodsEntity));
    }

	@Override
	public List<Map<String, Object>> getOrderGoods(Integer orderSeq) throws Exception {
    	Map<String,Object> map = new HashMap<>(10);
    	map.put("meetingOrderSeq",orderSeq);
