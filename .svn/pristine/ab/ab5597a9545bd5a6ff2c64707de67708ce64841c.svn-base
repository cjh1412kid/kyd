...
Page({
  ...
  onLoad: function(options) {
    ...
    var cavW = wx.getSystemInfoSync().windowWidth;
    QR.api.draw("pages/order/detail/detail?seq=" + seq, "myCanvas", 0.5 * cavW, 0.5 * cavW);
  ...
  paySuccess:function(e){
      var that=this;
      var paidMoney = that.data.paidMoney;
      var seq = that.data.orderDetail.seq;
      if (!paidMoney){
        app.gwToast('支付金额未填写');
        return;
      }
