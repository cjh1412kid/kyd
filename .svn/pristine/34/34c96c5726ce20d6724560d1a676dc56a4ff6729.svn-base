  ...
  data: {
    ...
    ths: [
     ...
     // {name: "订单状态", value: "orderStatus", color: "blue", lineFieldFlag: true},
      ...
      {name: "收件人", value: "receiverName", color: "blue", lineFieldFlag: true},
      {name: "收件人电话", value: "telephone", color: "blue", lineFieldFlag: true},
      {name: "收件人地址", value: "address", color: "blue", lineFieldFlag: true},
      {name: "物流名称", value: "expressName", color: "blue", lineFieldFlag: true},
      ...
      {name: "厂家", value: "factory", color: "blue", lineFieldFlag: true},
      {name: "厂家货号", value: "factoryGoodID", color: "blue", lineFieldFlag: true},
      {name: "厂家价格", value: "factoryPrice", color: "blue", lineFieldFlag: true},
      {name: "订货会", value: "periodName", color: "blue", lineFieldFlag: true},
      {name: "单价", value: "productPrice", color: "blue", lineFieldFlag: true},
      {name: "时间", value: "inputTime",color: "blue", lineFieldFlag: true},
      /*汇总字段*/
    ...
    keywords: null,
    modelMeetingSeqChecked: -1,
    orderMeetings:[],
    modelUserSeqChecked: -1,
    orderUsers:[],
  },
  methods: {
    /*初始化表格*/
    ...
    chooseSelect:function(data){
    	var meetingSeq=data;
    	/*根据meetingSeq查询订货方*/
   	 $.get(baseURL + "system/meetingStatisticsModel/allUserList?meetingSeq="+meetingSeq, function (data) {
 	
   		 if (data.code == 0) {
 		    vm.orderUsers = data.result;
 		   vm.modelUserSeqChecked=-1;
 		}
 	});
    },
    /*data格式： [[{cell},{cell}],[{},{}]]*/
...
function initOrderModelList() {
  ...
  $.get(baseURL + "system/meetingStatisticsModel/allList", function (data) {
    if (data.code == 0) {
      vm.orderModels = data.result;
    }
  });
}
function initOrderMeetings(){
	 $.get(baseURL + "system/meetingStatisticsModel/allMeetingList", function (data) {
		if (data.code == 0) {
		    vm.orderMeetings = data.result;
		}
	});
};
...
function queryOrderData() {
  ...
  if (vm.modelSeqChecked > -1) {
    ...
    if (vm.modelMeetingSeqChecked > -1) {
      queryCondition += "&meetingSeq=" + vm.modelMeetingSeqChecked;
    }

 

    if (vm.modelUserSeqChecked>-1) {
      queryCondition += "&userSeq=" + vm.modelUserSeqChecked;
    }

    console.log("queryCondition: ",queryCondition)
    $.get(baseURL + "meetingStatistics/list?statisticsModelSeq=" + vm.modelSeqChecked + queryCondition, function (r) {
    	if (r.code == 0) {
        if (r.lineField) {
...
//    queryParam += "meetingSeq=" + vm.modelMeetingSeqChecked;
//  }
//
//  if (vm.modelUserSeqChecked>-1) {
//    queryParam = queryParam == "" ? queryParam + "?" : queryParam + "&";
//    queryParam += "userSeq=" + vm.modelUserSeqChecked;
//  }
//  var excelForm = document.getElementById('exportOrderExcelForm');
//  excelForm.action = baseURL + "meetingStatistics/exportExcel" + queryParam;
