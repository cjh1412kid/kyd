...
//	          }
            ...
            case 7:
              if (hasPermission("order:cancelOrder")) {
                buttonHtml += ('<button class="operation-btn-dangery" onclick="deleteOrder(' + rowObject.seq + ')">删除</button>' + '&nbsp;');
              }
              return buttonHtml;
          }
        },
...
//删除订单
function deleteOrder(seq) {
  confirm("确定要删除订单？ 删除后不可恢复！", function () {
	 $.post(baseURL + "order/deleteOrder", {
	   seq: seq
	 }, function (r) {
	   if (r.code === 0) {
	 	alert('操作成功', function () {
