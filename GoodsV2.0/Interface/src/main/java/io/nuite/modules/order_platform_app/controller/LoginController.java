	...
	public R login(String accountName, String password) {
		try {
			...
			baseUser.setHeadImg(getBaseUserPictureUrl(baseUser.getSeq().toString()) + baseUser.getHeadImg());
				//添加公司名称
			BaseCompanyEntity companyEntity = baseCompanyDao.selectById(baseUser.getCompanySeq());
			baseUser.setCompanyName(companyEntity.getName());
			baseUser.setProductVersion(companyEntity.getVersion());
				//添加工厂管理员seq
			...
			logger.error(baseUser.getUserName() + "在   " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss") + "   登陆了");
